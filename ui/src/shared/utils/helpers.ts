...
export const getUserFriendlyApiErrorMessage = (error: any): string => {
  try {
    if (!(error instanceof AxiosError)) {
      return error?.message ?? Errors.unknownMessage;
    }
    ...
    if (status === 401 || status === 403) {
      switch (data?.detail) {
        ...
        case "LockedOut":
          parts.push(
            `(Email address is locked out for ${AccountSecurity.lockOutTimeInMinutes} minutes after ${AccountSecurity.maxFailedAttemptsToSignIn} failed attempts to sign in)`,
          );
          break;
        case "InvalidToken":
        case "TokenExpired":
    ...
    return String(data ?? error.message ?? Errors.unknownMessage);
  } catch {
