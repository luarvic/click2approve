...
const formatHash = (hashValue: string): string => hashValue.match(/.{1,8}/g)?.join(" ") ?? hashValue;
...
const formatParticipantOrganization = (organizationDisplayName?: string): string => organizationDisplayName || "N/A";
...
const renderParticipant = (participant: SharedVerificationReceipt["participants"][number]) => {
  const assignee = <DisplayName allowDisplayNameWrap displayName={participant.displayName} email={participant.email} />;
...
const renderField = (label: string, value?: string | React.ReactNode | null, sx: SxProps<Theme> = valueSx) => (
          ...
          <Stack spacing={StackSpacing.loose} sx={certificateSx}>
            <Box sx={headerSx}>
              <Stack spacing={StackSpacing.default}>
                ...
                <Typography>
                  This certificate records the successful completion of the request identified below and the files
                  associated with it at the time of completion.
                </Typography>
              ...
              <Box sx={qrPanelSx}>
                ...
                <Typography sx={qrLabelSx}>Scan the QR code to verify this certificate</Typography>
              </Box>
            </Box>
              ...
              <Table size="small" sx={filesTableSx}>
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell align="right" sx={[tableHeaderCellSx, fileSizeColumnSx]}>
                      Size
                    </TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {receipt.files.map((file, index) => (
                    <TableRow key={file.globalId} sx={index === receipt.files.length - 1 ? tableLastRowSx : undefined}>
              ...
              <Table size="small" sx={participantTableSx}>
                ...
                <TableBody>
                  {(receipt.participants ?? []).map((participant, index) => (
                    ...
                    >
                      <TableCell sx={[tableCellSx, participantColumnSx]}>{renderParticipant(participant)}</TableCell>
                      ...
                      <TableCell align="right" sx={[tableCellSx, participantPerformedAtColumnSx]}>
                        {participant.completedAt ? formatCertificateDateTime(participant.completedAt) : "Not recorded"}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              ...
              <Stack spacing={StackSpacing.default} sx={mobileRecordStackSx}>
                {(receipt.participants ?? []).map((participant, index) => (
                  ...
                  >
                    ...
                    {renderField("Action", renderParticipantAction(participant.action))}
                    {renderField(
                      "Performed at",
                      participant.completedAt ? formatCertificateDateTime(participant.completedAt) : undefined,
                    )}
                  </Stack>
                ))}
              </Stack>
